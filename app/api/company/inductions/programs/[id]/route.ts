...
export async function PATCH(
  ...
  if (!canConfigureInductions(auth.role)) {
    return NextResponse.json({ error: "Only admins and managers can update induction programs." }, { status: 403 });
  }
  ...
  if (typeof body.audience === "string") {
    const a = body.audience.trim().toLowerCase();
    if (a === "worker" || a === "visitor" || a === "subcontractor") patch.audience = a;
  }
  if (Array.isArray(body.requiredDocs)) patch.required_docs = body.requiredDocs;
  ...
  if (res.error) {
    return NextResponse.json({ error: res.error.message || "Failed to update program." }, { status: 500 });
  }
  if (!res.data) {
    return NextResponse.json({ error: "Program not found." }, { status: 404 });
  }

  return NextResponse.json({ program: res.data });
}

...
export function ExcelTemplateByCategory({
  ...
  if (sections.length === 0) {
    return <p className="text-sm text-slate-300">No template sections loaded.</p>;
  }
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(200px,260px)_1fr]">
              ...
              <button
                ...
                className={`flex w-full flex-col gap-0.5 rounded-xl border px-3 py-2.5 text-left text-sm transition ${
                  active
                    ...
                    : "border-transparent bg-slate-900/90 text-slate-200 hover:border-slate-600/80 hover:bg-slate-900/90"
                }`}
              >
                <span className="leading-snug">{titleAt(sIdx)}</span>
                <span
                  className={`text-xs font-semibold uppercase tracking-wide ${active ? "text-emerald-100/90" : "text-slate-300"}`}
                >
                  {tabPrefix === "hs" ? "H&S" : "Env."} · Block {sIdx + 1}/{sections.length}
                </span>
                <span className={`text-xs font-normal ${active ? "text-emerald-100/85" : "text-slate-300"}`}>
      ...
      <div className="min-w-0 rounded-2xl border border-slate-700/80 bg-slate-900/90 p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700/60 pb-3">
          <div>
            <h3 className="text-base font-bold text-slate-50">{titleAt(safeIdx)}</h3>
        ...
        <div className="mt-4 space-y-3">
          {currentRows.length === 0 ? (
            <p className="rounded-xl border border-dashed border-slate-600/80 bg-slate-950/70 py-10 text-center text-sm text-slate-300">

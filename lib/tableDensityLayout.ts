...
export function listSectionDensity(isCompact: boolean) {
  return {
    ...
    card: isCompact
      ? "rounded-2xl border border-[var(--app-border-strong)] bg-[linear-gradient(180deg,_rgba(255,255,255,0.96)_0%,_var(--app-panel-soft)_100%)] p-3 shadow-[0_8px_18px_rgba(76,108,161,0.05)]"
      : "rounded-2xl border border-[var(--app-border-strong)] bg-[linear-gradient(180deg,_rgba(255,255,255,0.96)_0%,_var(--app-panel-soft)_100%)] p-4 shadow-[0_8px_18px_rgba(76,108,161,0.05)]",
    cardTitle: isCompact ? "text-sm font-semibold text-[var(--app-text-strong)]" : "text-sm font-semibold text-[var(--app-text-strong)]",
    cardMeta: isCompact ? "text-[11px] text-[var(--app-muted)]" : "text-xs text-[var(--app-muted)]",
    statGrid: isCompact ? "mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4" : "mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4",
    statCell: isCompact
      ? "rounded-xl border border-[var(--app-border)] bg-white/82 p-2.5"
      : "rounded-xl border border-[var(--app-border)] bg-white/82 p-3",
    statLabel: isCompact
      ? "text-[10px] uppercase tracking-[0.2em] text-[var(--app-muted)]"
      : "text-[11px] uppercase tracking-[0.22em] text-[var(--app-muted)]",
    statValue: isCompact ? "mt-0.5 text-sm font-semibold text-[var(--app-text-strong)]" : "mt-1 text-sm font-semibold text-[var(--app-text-strong)]",
  };
}

...
export function PermitCopilotPanel({
  ...
  const [undoSnapshotScopeKey, setUndoSnapshotScopeKey] = useState<string | null>(null);
  const undoScopeKey = selectedActivity?.id ?? currentDraft.jsaActivityId ?? null;
  const hasUndoSnapshot = Boolean(undoScopeKey && undoSnapshotScopeKey === undoScopeKey);

  const clearUndoState = useCallback(() => {
    lastAutoAppliedSnapshot.current = null;
    setUndoSnapshotScopeKey(null);
  }, []);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      if (Object.keys(patch).length > 0) {
        ...
        setUndoSnapshotScopeKey(undoScopeKey);
        onApply(patch);
  ...
  }, [clearUndoState, currentDraft, onApply, prompt, promptSeed, selectedActivity, selectedJobsiteName, undoScopeKey]);
  ...
  }, [currentDraft, onApply, suggestion, undoScopeKey]);
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              type="button"
              onClick={hasUndoSnapshot ? undoAutoFill : applySuggestion}
              disabled={!suggestion && !hasUndoSnapshot}
              className="app-btn-secondary rounded-xl px-4 py-2.5 text-sm disabled:cursor-not-allowed disabled:opacity-50"
            >
              {hasUndoSnapshot ? "Undo auto-fill" : "Apply suggestions"}
            </button>
            {selectedActivity ? (

...
import { InlineMessage, PageHero, SectionCard, StatusBadge, WorkflowPath } from "@/components/WorkspacePrimitives";
...
export function SafetyIntelligenceWorkflow({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const jobsiteQuery = jobsiteId ? `?jobsiteId=${encodeURIComponent(jobsiteId)}` : "";
  ...
  const analyticsHref = jobsiteId ? `/jobsites/${jobsiteId}/analytics` : "/analytics/safety-intelligence";
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <TradeTaskIntakeForm trades={dashboard?.trades ?? []} onSubmit={handleIntake} initialJobsiteId={jobsiteId} />
      </SectionCard>
      ...
      <SectionCard
        ...
        aside={<StatusBadge label={latestIntake ? "Evaluated" : "Pending intake"} tone={latestIntake ? "success" : "warning"} />}
      >

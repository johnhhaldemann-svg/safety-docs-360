...
import { buildPreTaskChecklist } from "@/lib/safety-intelligence/engine/preTaskChecklist";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const checklist = buildPreTaskChecklist(reviewContext);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to build checklist." },
      { status: 400 }
    );
  }
}
